...
@onready var default_data = preload("res://Player/Movement Data/DefaultMovementData.tres")
@onready var crouch_data = preload("res://Player/Movement Data/CrouchMovementData.tres")
@onready var dash_data = preload("res://Player/Movement Data/DashMovementData.tres")
@onready var wall_data = preload("res://Player/Movement Data/WallMovementData.tres")
...
func _physics_process(delta):
	...
	if state == "free":
		...
		#Reset variables to free
		movement_data = default_data
		collision_shape_2d.disabled = false
		hazard_collision.disabled = false
		animated_sprite_2d.z_index = 0
		
		#Reset particles and rotation
		...
		#Stop ghost and particles if not fast enough
		...
		#Wall Check
		if is_on_wall_only() and dash_count > 0:
			state = "wall"
			if velocity.y > 0 : movement_data = wall_data
...
func handle_jump():
	var wave_jump = 1.0
	if is_on_floor(): jump_count = 1
	elif coyote_jump_timer.time_left <= 0.0: jump_count = 0
	if state != "crouch":
...
func wall_slide(delta, input_axis):
	if velocity.y > 0 :
		velocity.y = 50
		movement_data = wall_data
	else:
		velocity.y += gravity * movement_data.gravity_scale * delta
	...
	if Input.is_action_just_pressed("jump") and jump_count > -1:
		...
		velocity.x = 380 * get_wall_normal().x
		jump_count -= 1
		state = "free"
...
func play_jump(wave_jump):
	...
	b_hop_sound.pitch_scale = rng.randf_range(0.9, 1.3)
	if wave_jump > 1.0:
		b_hop_sound.play()
